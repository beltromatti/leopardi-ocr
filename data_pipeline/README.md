...
- peak disk usage during the full pretraining-family build: plan for about `2.6-3.2 TB`
- minimum realistic free space: about `3.0 TB`
- recommended free space: about `3.5 TB`
...
- the largest peak occurs while `p2_exact_core_v1` is retained as upstream for `synthetic_from_exact` and `p3_hardcases_v1` is being written
- HF parquet-backed sources and internal synthetic generators still have near-zero local raw footprint relative to PDF-page corpora
- the `~4.5M` hard cases in `synthetic_from_exact` remain derived and are now generated by deterministic internal builders after exact-core publication, not mixed into the first external raw acquisition wave
...
- build and publish `sft_core_v1`, `f0_general_sft_v1`, `f1_specialist_sft_v1`, and `o_calibration_docmix_v1`
...
- deterministic internal builders for `approved_exact_full_page_targets`, `synthetic_from_exact`, `model_failures_plus_exact_truth`, and `sft_and_repair_prompt_packs`
- strict manual-manifest import workers only for curated holdouts or truly manual local corpora
